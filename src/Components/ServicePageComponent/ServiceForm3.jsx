...
const ServiceForm3 = () => {
 ...
 e.target.value = "";
    ...
    // Append files
    totalFiles.forEach((file) => formData.append("membershipFiles", file));

    // Append other data
    formData.append("facultyName", username);
    formData.append("membership ", roles);

    try {
      const response = await axios.post(`${API}/api/membership/${designation}/`, formData, {
        headers: {
  ...
  const handleSubmit = async (value) => {
    ...
    try {
      const response = await axios.post(`${API}/api/membership/${designation}/`, {
        membership: value,

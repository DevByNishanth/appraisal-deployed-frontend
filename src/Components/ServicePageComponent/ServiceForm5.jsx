...
const ServiceForm5 = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    // Append files
    totalFiles.forEach((file) => formData.append("administrationFiles", file));

    // Append other data
    formData.append("facultyName", username);
    formData.append("assistance ", roles);

    try {
      const response = await axios.post(`${API}/api/assistance/${designation}/`, formData, {
        headers: {
  ...
  const handleSubmit = async (value) => {
    ...
    setRoles(value);
    try {
      const response = await axios.post(`${API}/api/assistance/${designation}/`, {
        assistance: value,
        ...
        <div className="second-container col-span-2 text-center">
          ...
          <div className="h-[80%] flex items-center justify-center">
            <h1 className="text-[#646464]  text-lg">
              ...
              of {serviceMarks?.Administration}
            </h1>
          </div>
        </div>
      </div>

...
const ServiceForm2 = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    // Append files
    totalFiles.forEach((file) => formData.append("brandingFiles", file));

    // Append other data
    formData.append("facultyName", username);
    formData.append("branding ", roles);

    try {
      const response = await axios.post(`${API}/api/branding/${designation}/`, formData, {
        headers: {
  ...
  const handleSubmit = async (value) => {
    ...
    try {
      const response = await axios.post(`${API}/api/branding/${designation}/`, {
        branding: value,
          ...
          <div className="checkbox-container px-6 mt-4 space-y-2">
            ...
            <div className="container-1 flex gap-2 items-center">
              <input
                ...
                onChange={() => { setSelectedCheck("No"), handleSubmit("No") }}
              />
        ...
        <div className="second-container col-span-2 text-center">
          ...
          <div className="h-[80%] flex items-center justify-center">
            <h1 className="text-[#646464]  text-lg">
              <span className="font-semibold text-[#318179]">{mark || 0}</span> out of {serviceMarks?.Branding}
            </h1>
          </div>
        </div>
      </div>

...
const ResearchForm3 = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    totalFiles.forEach((file) => {
      formData.append("AicteFiles", file); // field name must match backend
    });
  ...
  async function sendDataToAPI(value) {
    const formData = new FormData();
    formData.append("aicte", JSON.stringify(value));
    formData.append("facultyName", username);
              ...
              <>
                {inputGroups.map((item, index) => {
                  return (
                    <div
                      key={index}
                      className="input-container md:mx-6 mt-2 space-y-2 w-full  flex flex-wrap items-center justify-between gap-4"
                    >
                      <input
                        type="text"
                        placeholder="Name of the paper"
                        className="border-1 border-gray-400 rounded-md px-4 py-2 w-[80%] md:w-[50%]"
                        value={item.author}
                        maxLength={20} // 👈 limits input to 16 characters
                        onChange={(e) => {

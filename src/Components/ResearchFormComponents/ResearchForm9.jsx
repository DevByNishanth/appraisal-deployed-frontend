...
const ResearchForm9 = () => {
 const API =import.meta.env.VITE_API
  ...
  const handleCheckbox = async (value) => {
    console.log("running 2.9 index")
    const formData = {};
    formData.citation = value;
    formData.facultyName = username;  // plain JSON

    
    setSelectedCheck(value);
    try {
      const response = await axios.post(`${API}/api/Citation/${designation}`, formData, {
        headers: {
  ...
  const handleFileUpload = async (e) => {
    ...
    // Append files
    totalFiles.forEach((file) => formData.append("citationFiles", file));

    // Append other data
    formData.append("facultyName", username);
    formData.append("citation", selectedCheck);


    try {
      const response = await axios.post(`${API}/api/Citation/${designation}/`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="checkbox-container mt-2 px-6 space-y-2">
            ...
            <div className="container-1 flex items-center gap-2">
              <input
                type="radio"
                className="scale-125 cursor-pointer"
                checked={selectedCheck == "None" ? true : false}
                onChange={() => {
                  handleCheckbox("");
                }}
              />
        ...
        <div className="second-container col-span-2 text-center">
          ...
          <div className="h-[80%] flex items-center justify-center">
            <h1 className="text-[#646464]  text-lg">
              <span className="font-semibold text-[#318179]">{mark || 0}</span> out of {researchMarks?.citation}
            </h1>
          </div>
        </div>
      </div>

...
const ResearchForm11 = () => {
  ...
  const handleCheckbox = async (value) => {
    console.log("running consultancy index");
    const formData = {};
    ...
    try {
      const response = await axios.post(`${API}/api/consultancy/${designation}`, formData, {
        headers: {
  ...
  const handleFileUpload = async (e) => {
    ...
    // Append files
    totalFiles.forEach((file) => formData.append("consultancyFiles", file));

    // Append other data
    formData.append("facultyName", username);
    formData.append("consultancy", selectedCheck);
        ...
        <div className="second-container col-span-2 text-center">
          ...
          <div className="h-[80%] flex items-center justify-center">
            <h1 className="text-[#646464]  text-lg">
              <span className="font-semibold text-[#318179]">{mark || 0}</span> out of {researchMarks?.consultancy}
            </h1>
          </div>
        </div>
      </div>

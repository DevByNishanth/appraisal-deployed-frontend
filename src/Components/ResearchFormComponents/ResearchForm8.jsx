...
const ResearchForm8 = () => {
  ...
  const handleCheckbox = async (value) => {
    ...
    try {
      const response = await axios.post(`${API}/api/Iindex/${designation}`, formData, {
        headers: {
  ...
  const handleFileUpload = async (e) => {
    ...
    // Append files
    totalFiles.forEach((file) => formData.append("IindexFiles", file));

    // Append other data
    formData.append("facultyName", username);
    formData.append("Iindex", selectedCheck);


    try {
      const response = await axios.post(`${API}/api/Iindex/${designation}/`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,

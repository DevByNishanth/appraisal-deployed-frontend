...
const ResearchForm14 = () => {
  ...
  // function for handling radio button click and api call while the button is clicked
  const handleCheckbox = async (value) => {
    console.log("running seedfund index");
    const dataFormData = {};
    dataFormData.seedFund = value;
    setSelectedCheck(value);
    dataFormData.facultyName = username;
    try {
      const response = await axios.post(`${API}/api/SeedFund/${designation}`, dataFormData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      console.log("res seedfund index : ", response);
  ...
  // function for handling file uploads and api call while the file is uploaded
  const handleFileUpload = async (e) => {
    ...
    // Append files
    totalFiles.forEach((file) => formData.append("seedFundFiles", file));

    // Append other data
    formData.append("facultyName", username);
    formData.append("seedFund", selectedCheck);

    try {
      const response = await axios.post(`${API}/api/SeedFund/${designation}/`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        ...
        <div className="first-container pr-3 border-r border-gray-400 col-span-10">
          ...
          <div>
            <h1 className="text-lg font-medium">
               Seed fund Activities <span className="text-red-500">*</span>
            </h1>
          </div>
          {/* input container  */}
          <div className="checkbox-container mt-2 px-6 space-y-2">
            ...
            <div className="container-1 flex items-center gap-2">
              <input
                ...
                className="scale-125 cursor-pointer"
                checked={
                  selectedCheck == "greater than two lakh" ? true : false
                }
                onChange={() => {
                  handleCheckbox("greater than two lakh");
                }}
              />
              <label className="text-[#6f7282]">More than Rs. 2 Lakhs</label>
            </div>
            <div className="container-1 flex items-center gap-2">
              <input
                type="radio"
                className="scale-125 cursor-pointer"
                checked={
                  selectedCheck == "Research Publications" ? true : false
                }
                onChange={() => {
                  handleCheckbox("Research Publications");
                }}
              />
              <label className="text-[#6f7282]">Research Publications</label>
            </div>
            <div className="container-1 flex items-center gap-2">
              <input
                type="radio"
        ...
        <div className="second-container col-span-2 text-center">
          ...
          <div className="h-[80%] flex items-center justify-center">
            <h1 className="text-[#646464]  text-lg">
              <span className="font-semibold text-[#318179]">{mark || 0}</span> out
              of {researchMarks?.seedfund}
            </h1>
          </div>
        </div>
      </div>
